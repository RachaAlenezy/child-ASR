...
for i, p in enumerate(wavs):
    ...
    if (llabel in one):
        ...
        llabel = 6
    elif (llabel in seven):
        llabel = 7
    elif (llabel in eight):
        llabel = 8
    else:
        llabel = 9
    # win length 79!!!
    print("group: ", llabel)
    mfcc_features = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13, win_length=256).T
...
#     if dlabels[i] not in all_labels:
#         all_labels.append(dlabels[i])
#     if i % 50 == 0:
#         test.append(mfccs[i])
#         test_labels.append(dlabels[i])
#     else:
#         train.append(mfccs[i])
#         train_labels.append(dlabels[i])
# hmms = []
#
